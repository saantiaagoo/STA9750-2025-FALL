...
As you could see, despite public perception, on average, the rent burden for New Yorkers tends to be lower than the benchmark.
```{r}
#| echo: false
#| message: false
#| warning: false
...
# ---- assume income_rent and cbsa_housing exist as in prior chunk ----
# If not, you can reuse the "self-contained" build I sent earlier.

# 1) Join and FIX the duplicate cbsa_name columns
cbsa_merged <- inner_join(
  income_rent,                                             # cbsa_code, cbsa_name, year, rent_burden_raw
  cbsa_housing,                                            # cbsa_code, cbsa_name, year, population, housing_growth_composite_std
  by = c("cbsa_code","year"),
  suffix = c(".rent", ".house")
) %>%
...
# Dynamic windows for start/end (first/last 3 years present)
...
# --- 2️⃣ Line Graph — New York CBSA Only (code 35620) ---
ny_code <- "35620"
...
# --- Visualization: NYC (2014–2018) ---
ggplot(ny_data, aes(x = year)) +
  # Solid line = rent burden
  geom_line(aes(y = rent_burden * 100, color = "Rent Burden"), linewidth = 1.3) +
  # Dashed line = housing growth (true standardized values)
  geom_line(aes(y = housing_growth, color = "Housing Growth"), linewidth = 1.3, linetype = "dashed") +
  
  # Custom Y-axis range and breaks
  scale_y_continuous(
    name = "Rent Burden (% of Income Spent on Rent)",
    limits = c(-10, 30),                      # y-axis from −10 to 30
    breaks = seq(-10, 30, by = 10),           # ticks at −10, 0, 10, 20, 30
  ...
  labs(
    ...
    subtitle = "Solid = Rent Burden; Dashed = Housing Growth (true standardized scale)",
    x = "Year"
  ) +
  theme_minimal(base_size = 13) +
  theme(
...
# === VIS C: YIMBY identification (unchanged, now works with fixed names) ===
...
ggplot(cbsa_summary,
  ...
  labs(
    ...
    subtitle = sprintf("Start window: %s–%s | End window: %s–%s",
                       min(start_years), max(start_years), min(end_years), max(end_years)),
