...
First, we are going to familiarize ourselves with the data. Below, you will see the relationships between all of our data tables and how they can be related to one another.
```{r}
#| echo: false
#| message: false
#| warning: false
...
state_income_2015 <- INCOME |>
  ...
  left_join(HOUSEHOLDS |> select(GEOID, households, year),
            by = c("GEOID", "year")) |>
  ...
  mutate(
    state_name_lower = tolower(name) 
  )

us_map <- map_data("state")  
us_map_income <- us_map |>
  left_join(
    state_income_2015,
...
#| out.width: "100%"
#| fig-align: center
ggplot(us_map_income, aes(long, lat, group = group, fill = avg_individual_income_num)) +
...
The map above shows all 50 states average income. However, it does not include additional US territories that are also included in the data. Therefore, more analysis has to be completed. That's when you figure out that DC had the highest average individual income for 2015, coming in at . Something, people in the government should be familiar too.
...
We look at various years to find no trend. However, we are able to find the peak in the year `r nyc_peak$YEAR`, with a `r nyc_peak$finance_share`%. 