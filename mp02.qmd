...
We highlighted two specific CBSAs, the NY and LA, to take a closer look at the evolution.
...
# 3) Center & standardize for interpretability
#    Center at 0.30 (30% benchmark), scale by 0.05 so ±1 ≈ ±5 percentage points
income_rent <- income_rent |>
  mutate(
...
ggplot(nyc_rent_burden, aes(x = year, y = rent_burden_raw)) +
  ...
  labs(
    title = "Rent Burden Over Time — New York CBSA",
    x = "Year",
    y = "Rent-to-Income Ratio",
    caption = "Dashed red line = 30% affordability benchmark"
  ) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))
```
In the visualization above, we take a look at the rent burden in the CBSA in New York. To understand better, the dashed red line represents the 30% affordability benchmark.

As you could see, despiite public perception, on average, the rent burden for New Yorkers tends to be lower than the benchmark.
```{r}
#| echo: false
#| message: false
#| warning: false
...
We are highlighting the CBSA's with the highest and lowest rent burdens.

Next, we are going to look at housing growth within the CBSAs.

We are going to construct two metrics, one will allow us to see the housing growth based on population of a CBSA and number of new housing units permitted for the year.

The other metric will compare the housing permits to the population growth over a 5 year lookback windown.
...
cbsa_housing <- POPULATION |>
  ...
  mutate(cbsa_code = str_replace_all(GEOID, "[^0-9]", ""),
         cbsa_name = str_remove(NAME, ",.*")) |>
  left_join(
    PERMITS |>
...
cbsa_housing <- cbsa_housing |>
  mutate(
    housing_growth_instant_std = (housing_growth_instant - mean(housing_growth_instant, na.rm = TRUE)) /
                                 sd(housing_growth_instant, na.rm = TRUE),
    housing_growth_rate_std = (housing_growth_rate - mean(housing_growth_rate, na.rm = TRUE)) /
                              sd(housing_growth_rate, na.rm = TRUE)
  )
top_instant <- cbsa_housing |>
  filter(!is.na(housing_growth_instant_std)) |>
  arrange(desc(housing_growth_instant_std)) |>
  distinct(cbsa_code, cbsa_name, .keep_all = TRUE) |>
  slice_head(n = 10) |>
...
top_rate <- cbsa_housing |>
  filter(!is.na(housing_growth_rate_std)) |>
...
DT::datatable(top_instant,
          caption = "Top 10 CBSAs by Instantaneous Housing Growth (Standardized)",
          options = list(pageLength = 10))

DT::datatable(bottom_instant,
          caption = "Bottom 10 CBSAs by Instantaneous Housing Growth (Standardized)",
          options = list(pageLength = 10))
```
...
DT::datatable(top_rate,
          caption = "Top 10 CBSAs by Rate-Based Housing Growth (Standardized)",
          options = list(pageLength = 10))

DT::datatable(bottom_rate,
          caption = "Bottom 10 CBSAs by Rate-Based Housing Growth (Standardized)",
          options = list(pageLength = 10))
```
...
Once we figured out the metric, we are going to identify the CBSA's that do well and poor with this measure
```{r}
#| echo: false
#| message: false
#| warning: false
...
library(scales)
library(DT)
library(slider)  # <-- use slider for safe rolling means

# 1) Join POPULATION and PERMITS at CBSA + year
cbsa_housing <- POPULATION |>
  select(GEOID, NAME, year, population) |>
  mutate(
    cbsa_code = str_replace_all(GEOID, "[^0-9]", ""),
...
# 2) 5-year lookback population using lag() within CBSA
cbsa_housing <- cbsa_housing |>
  group_by(cbsa_code) |>
  mutate(
    population_lag5 = lag(population, 5),
...
# 3) Construct the two housing-growth metrics
cbsa_housing <- cbsa_housing |>
  mutate(
    # Instantaneous: permits per person this year
    housing_growth_instant = new_housing_units_permitted / population,

    # Rate-based: permits per *incremental* resident over prior 5 years
...
# 4) Standardize (z-score) both metrics across all CBSA-years
cbsa_housing <- cbsa_housing |>
  mutate(
    housing_growth_instant_std = (housing_growth_instant - mean(housing_growth_instant, na.rm = TRUE)) /
                                 sd(housing_growth_instant, na.rm = TRUE),
...
# 5) Composite score (equal-weighted) + standardized composite
cbsa_housing <- cbsa_housing |>
  mutate(
    housing_growth_composite = 0.5 * housing_growth_instant_std +
                               0.5 * housing_growth_rate_std,
    housing_growth_composite_std = (housing_growth_composite - mean(housing_growth_composite, na.rm = TRUE)) /
                                   sd(housing_growth_composite, na.rm = TRUE)
  )

# 6) Rolling 5-year averages (current year + previous 4) for standardized scores
#    Use slider so short series return NA instead of error; .complete=TRUE requires full 5.
cbsa_housing <- cbsa_housing |>
  group_by(cbsa_code) |>
  arrange(cbsa_code, year) |>
  mutate(
...
# 7) Top/Bottom tables (use the latest year to avoid duplicate CBSAs)
