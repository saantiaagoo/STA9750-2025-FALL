...
# --- Compute average revision per year ---
rev_yearly <- ces_revisions %>%
  mutate(year = year(date)) %>%
  group_by(year) %>%
  summarize(avg_revision = mean(revision, na.rm = TRUE), .groups = "drop")

# --- Categorize for highlighting ---
...
# --- Function: compute jobs added for a single presidency ---
...
# --- Apply function to each presidency ---
...
# Compute jobs added in the first ~100 days
