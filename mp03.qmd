...
Once we have visualized all the districts, next we are going to visualize the trees that exist within each one.
```{r}
#| echo: false
#| message: false
#| warning: false
...
Next, we will run code to figure out the districts with the highest density of trees.
```{r}
#| code-fold: true
...
ggplot() +
  ...
  geom_sf(data = d2_dead_trees, color = "red", alpha = 0.7, size = 0.6) +
  coord_sf(
  ...
  labs(
    title = "Dead Street Trees in District 2",
...
In the visualization above, you are able to see all the dead trees located around District 2. The district contains *`r d2_total_trees_fmt`* trees, of which *`r d2_dead_trees_fmt`* (about *`r d2_fraction_dead_pct`*) are dead.
...
density_compare <- district_stats %>%
  filter(CounDist %in% compare_dists) %>%
  mutate(
    district_label = if_else(CounDist == 2, "District 2", "Other District")
  )

ggplot(density_compare,
       aes(x = factor(CounDist),
           y = density_per_km2,
           fill = district_label)) +
  geom_col() +
  scale_fill_manual(
    values = c("District 2" = "darkgreen", "Other District" = "grey70")
  ) +
  scale_y_continuous(labels = comma_format()) +
  labs(
    title = "Tree Density Across Districts 1–4",
    x = "Council District",
    y = "Tree Density (per km²)"
  ) +
  theme_minimal() +
  theme(
...
ggplot(dead_compare,
  ...
  geom_col() +
  scale_fill_manual(
    values = c("District 2" = "firebrick", "Other District" = "grey70")
  ) +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title = "Fraction of Dead Trees Across Districts 1–4",
    x = "Council District",
...
District 2 is the most dense district with trees compared to nearby districts. To go along with the high density, it has the most amount of dead trees, therefore resulting in the highest fraction out of the 4. Below now is a visualization to aid the reader visualize the results from the graphs above on a map.
```{r}
#| echo: false
#| message: false
#| warning: false
